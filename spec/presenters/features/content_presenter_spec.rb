...
RSpec.describe ContentPresenter, :type => :decorator do
  ...
  describe '#form_partial_path' do
    it { expect(instance).to have_reader(:form_partial_path).with('features/contents/fields') }

    context 'with a text content', :content => :text do
      it { expect(instance.form_partial_path).to be == 'features/contents/text_contents/fields' }
    end # context
  end # describe
